<!-- Auto-generated by x/build/update-readmes.go -->

[![Go Reference](https://pkg.go.dev/badge/golang.org/x/build/cmd/maintnerd.svg)](https://pkg.go.dev/golang.org/x/build/cmd/maintnerd)

# golang.org/x/build/cmd/maintnerd

The maintnerd command serves project maintainer data from Git, Github, and/or Gerrit.
<!-- End of auto-generated section -->

## Deployment

See the documentation on [deployment](../../doc/deployment.md).
