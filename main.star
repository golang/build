...
def make_run_mod(add_props = {}, add_env = {}, enabled = None, test_timeout_scale = 1):
    def apply_mod(props, project):
        ...
        if "GOEXPERIMENT" in e and "GOEXPERIMENT" in props["env"]:
            ...
            # of key=value pairs. Merge them. See https://pkg.go.dev/internal/goexperiment.
...
def define_builder(env, project, go_branch_short, builder_type):
    ...
    # Set GOEXPERIMENT=synctest everywhere for go1.24 and above.
    #
    # N.B. This must come before applying run mods, which may add more
    # GOEXPERIMENTs.
    if go_branch_short == "gotip" or go_branch_short >= "go1.24":
